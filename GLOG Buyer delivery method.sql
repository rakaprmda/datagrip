...
LEFT JOIN   "gada-marketplace".address_area province on province.id = city.parent_area_id
INNER JOIN   gada_logistic_buyer ON gada_logistic_buyer.unique_id = s.unique_id AND "order".created_at >= order_date
LEFT JOIN   (
            SELECT      ou.order_id
...
SELECT
    ...
    round(avg(glog_delivery/total_order),4) * 100 glog,
    round(avg(self_pickup/total_order),4) * 100 self_pickup,
    round(avg(store_courier/total_order),4) * 100 store_courier
