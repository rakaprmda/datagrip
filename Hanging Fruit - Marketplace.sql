WITH gada_logistic_buyer as (
    ...
    where
    cast(f.status_updated_at as char(7)) >= '2021-02' and cast(f.status_updated_at as char(7)) < '2021-03'
...
,all_buyer AS (
    SELECT      unique_id buyer_code,  city, province, gada_tag,address_latitude,address_longitude,delivery_method,COUNT(distinct order_id) count_order
    FROM        raw
...
,percentile as (
    SELECT      province, percentile
                , MIN(count_order) order_percentile
    FROM        (
                SELECT      province,
                            FLOOR(percent_rank() OVER(partition by  province ORDER BY count_order) * 10) / 10 percentile
                            , count_order
                FROM        (SELECT * FROM all_buyer WHERE count_order IS NOT NULL)x
                )y
...
--
    ...
    SELECT
        temp.province,
        percentile,
        percentile.order_percentile,
    --     count(distinct buyer_code) all_buyer,
        sum(case when temp.count_order >= percentile.order_percentile then 1 else 0 end) as percentile_Buyer,
        sum(case when (temp.count_order >= percentile.order_percentile AND gada_tag is not null) then 1 else 0 end) as gada_Buyer,
        sum(case when (temp.count_order >= percentile.order_percentile AND gada_tag is null) then 1 else 0 end) as non_gada_Buyer

    FROM
    all_buyer temp inner join percentile on percentile.province = temp.province
    group by 1,2,3
    HAVING sum(case when (temp.count_order > percentile.order_percentile AND gada_tag is not null) then 1 else 0 end) > 10
...
FROM
       all_buyer temp inner join percentile on percentile.province = temp.province
        AND temp.count_order > percentile.order_percentile AND gada_tag is null
